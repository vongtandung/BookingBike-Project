...
class LocateIdentify extends Component {
  constructor(props) {
    ...
    this.selfUserList =
      sessionStorage.getItem("userList") != null
        ? JSON.parse(sessionStorage.getItem("userList"))
        : [];
    this.selfUserNum =
      sessionStorage.getItem("userNum") != null
...
class Map extends Component {
  ...
  getPoint(event) {
    ...
    self.webService
      ...
      .then(res => {
        let address = "";
        if (res.plus_code.compound_code.indexOf("Vietnam") !== -1) {
        ...
        } else if (res.plus_code.compound_code.indexOf("Vietnam") === -1) {
          self.props.popup({
  ...
  >
    {props.userList.map((userMarker, index) => {
      return (
        <div key={index}>
          <Marker
            position={userMarker.center}
            icon={markerIco}
            label={userMarker.name + "-" + userMarker.phone}
            onClick={props.userSelect === index ? props.onMapClick : null}
          />
        </div>
      );
    })}
